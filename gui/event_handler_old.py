...
class EventHandler:
    ...
    def handle_click(self, pos: Tuple[int, int], game: Game) -> bool:
        ...
        # ────────────────────────────────────────────────
        # 3. Advancing from InitialPlacement to Execution
        # ────────────────────────────────────────────────
        if game.phase == "InitialPlacement" and game.ip.current_step >= len(game.ip.sequence):
            game.phase = "Execution"
            print("DEBUG: Initial Placement completed. Transitioning to Execution...")
            game.start_execution_phase()
            print("DEBUG: start_execution_phase completed.")
            self._redraw_after_execution_event(game)
            return True

        # ────────────────────────────────────────────────
        # 4. Execution-phase buttons / basic 7 robber click
        # ────────────────────────────────────────────────
