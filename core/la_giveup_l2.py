"""Phase L L6: god-view LA give-up → clear ambition + force L2.

Uses frozen Domain A thresholds from ``core.la_giveup_config`` / constants.
Gated by ``LA_GIVEUP_L2_ENABLED`` (lab default may be True for A/B batches).

Fire rule (safe profile)::

  needs_LA and not holds_LA
  hopeless_score_la ≥ θ for D consecutive own-turn strategy samples
  → latch first fire: clear sticky, force L2 explore, LA way-kill latch

Does not implement LR give-up (separate track).
"""
...
from core.la_giveup_config import (
    FREEZE_ID,
...
def _get_latch(player: Any) -> Dict[str, Any]:
    raw = getattr(player, "la_giveup_latch", None) if player is not None else None
...
def _set_latch(player: Any, payload: Optional[Mapping[str, Any]]) -> None:
    if player is None:
        return
    try:
        player.la_giveup_latch = dict(payload) if payload else None
    except Exception:
        pass
...
def _clear_run_state(player: Any) -> None:
    if player is None:
        return
    try:
        player.la_giveup_run_len = 0
    except Exception:
        pass
    try:
        player.la_giveup_last_score = None
    except Exception:
        pass
...
def _apply_giveup_mutation(
    ...
    *,
    ...
    """Clear sticky LA path + force L2 (same spine as S5b LA way-kill)."""
    ...
    try:
        ...
        invalidate_board_way_portfolio_cache(game, f"la_giveup:{reason}")
    except Exception:
        pass
    try:
    ...
    try:
        ...
        pass
    # Reuse way-kill latch so S5b / LA progress see "LA dead" for this episode
    try:
        set_way_kill_latch(
            player,
            way_id=way_id,
            kind="LA",
            game=game,
            reason=f"la_giveup_theta:{theta}",
        )
    except Exception:
        pass
    try:
    ...
    try:
        ...
        if is_giveup_escape_enabled():
            ...
            mark_giveup_fired_on_episode(player, "la", game=game)
    except Exception:
        pass
    # Hygiene: drop LA flags on direction so needs probe can leave LA sooner
    try:
        ...
        if isinstance(direction, dict):
            ...
            direction["preference_source"] = (
                str(direction.get("preference_source") or "") + "+la_giveup_pending"
            ).lstrip("+")
...
def maybe_la_giveup_l2(
    ...
    *,
    ...
    """Evaluate LA give-up on strategy refresh; fire once per latch episode.
    ...
    theta = float(profile.get("theta") or 0.6)
    ...
    # Build god-view LA block (same features as probe / offline score)
    try:
        from core.la_lr_probe_log import build_la_lr_probe_row

        row = build_la_lr_probe_row(
            game, player, reason=str(reason or "la_giveup_check"), event="la_giveup_check"
        )
        la = dict(row.get("la") or {}) if isinstance(row, Mapping) else {}
    except Exception as exc:
        out["reason"] = f"probe_build_error:{exc}"
        return out

    needs = bool(la.get("needs"))
    holds = bool(la.get("holds"))
    out["needs_LA"] = needs
    out["holds_LA"] = holds
    out["needs_reason"] = la.get("needs_reason")
    way_id = _sticky_way_id(player)
    out["way_id"] = way_id

    # WP1: release specials-dead episode when LA ambition ends or is held
    try:
        from core.specials_dead_episode import maybe_release_specials_dead_episode

        maybe_release_specials_dead_episode(
            game, player, holds_la=holds, needs_la=needs
        )
    except Exception:
        pass
    ...
    if not needs:
        ...
        # Clear latch when ambition ends so a future LA lock can fire again
        if _get_latch(player):
            _set_latch(player, None)
        out["reason"] = "no_needs_LA"
        out["skipped"] = True
        return out

    if holds:
        _clear_run_state(player)
        out["reason"] = "already_holds_LA"
        out["skipped"] = True
        return out

    try:
        from core.batch.la_lr_godview import hopeless_score_la

        score = float(hopeless_score_la(la))
    except Exception as exc:
    ...
    out["score"] = score
    try:
        player.la_giveup_last_score = score
    except Exception:
        pass

    # Option (2): one fire per specials-dead kill_la episode
    try:
        from core.specials_dead_episode import episode_blocks_giveup_refire

        if episode_blocks_giveup_refire(player, "la"):
            out["reason"] = "one_fire_per_episode"
            out["skipped"] = True
            out["one_fire_per_episode"] = True
            out["run_len"] = _safe_int(getattr(player, "la_giveup_run_len", 0), 0)
            return out
    except Exception:
        pass
    ...
    if latch_first and latch.get("active"):
        # Same sticky way episode — do not re-fire
        ...
        if latched_way is None or way_id is None or int(latched_way) == int(way_id):
            ...
            out["run_len"] = _safe_int(getattr(player, "la_giveup_run_len", 0), 0)
            return out
        # Way changed — clear way-latch; episode one-fire still applies above
        _set_latch(player, None)
        _clear_run_state(player)
    ...
    run_len = _safe_int(getattr(player, "la_giveup_run_len", 0), 0) + 1
    try:
        player.la_giveup_run_len = run_len
    except Exception:
        pass
    out["run_len"] = run_len
    out["skipped"] = False
    ...
    # Fire (capture run_len before reset; log probe row from pre-mutation snapshot)
    ...
    mut = _apply_giveup_mutation(
        ...
        detail={
            ...
            "needs_reason": la.get("needs_reason"),
            "gap": la.get("gap"),
            "n_threats": la.get("n_threats"),
            "army": la.get("army"),
            "kill_recommended": la.get("kill_recommended"),
        },
    )
    _set_latch(
        player,
    ...
    if game is not None:
        try:
            game.last_la_giveup = dict(player.last_la_giveup)
        except Exception:
            pass
    try:
        ...
        log_giveup_fire_event(
            ...
            reason=str(reason or "la_giveup_theta"),
